    ...
    */
    ...
    private void setDatabaseAttributes(Element database)
    {
        ...
        if (checkSyntax("loglevel",loglevel,new String[]{"trace","debug","info","warn","error","fatal"}))
        {
            ...
            {
                Logger.setLogLevel(Logger.INFO_ID);
            }
            ...
            {
                Logger.setLogLevel(Logger.ERROR_ID);
            }
            else if ("fatal".equalsIgnoreCase(loglevel))
            {
                Logger.setLogLevel(Logger.FATAL_ID);
            }
            else
            {
                Logger.error("Invalid loglevel. Should be one of: trace, debug, info, warn, error, fatal.");
            }
        }
        ...
        if (checkSyntax("caching",caching,new String[] {"none","no","false","yes","true","soft","growing","full"}))
        {
            ...
            if (val == Cache.FULL_CACHE)
            {
                Logger.warn("The 'full' caching method is deprecated and will be removed in future versions.");
            }
        }
        ...
        if (minstr != null)
        {
            try
            {
                ...
                if (min>0)
                {
                    this.database.setMinConnections(min);
                }
                else
                {
                    Logger.error("the parameter 'min-connections' wants an integer > 0 !");
                }
            }
            catch(NumberFormatException nfe)
            {
        ...
        if (maxstr != null)
        {
            try
            {
                ...
                if (max>=min)
                {
                    this.database.setMaxConnections(max);
                }
                else
                {
                    Logger.error("the parameter 'max-connections' must be >= min-connection!");
                }
            }
            catch(NumberFormatException nfe)
            {
    ...
    @SuppressWarnings("deprecation") private void defineAttributes(Element parent,Entity entity) throws SQLException
    {
        ...
        for(Element element:(List<Element>)attributes)
        {
            ...
            else if(element.getName().equals("rowset"))
            {
              ...
              if(entity != null)
              {
                  attribute.setResultEntity(adaptCase(resultEntity));
              }
            }
